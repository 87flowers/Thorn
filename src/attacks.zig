...
const masks_table = blk: {
    ...
    var result: [64]Masks = .{};
    for (0..64) |i| {
        const sq = Square{ .raw = i };
        result[i] = .{
            .diagonal = SquareSet.rayMask(sq, .ne).bitOr(SquareSet.rayMask(sq, .sw)),
            .antidiagonal = SquareSet.rayMask(sq, .nw).bitOr(SquareSet.rayMask(sq, .se)),
            .file = SquareSet.rayMask(sq, .n).bitOr(SquareSet.rayMask(sq, .s)),
            .rank = SquareSet.rayMask(sq, .e).bitOr(SquareSet.rayMask(sq, .w)),
        };
    }
    break :blk result;
};
