...
fn generatePawnCaptures(moves: *MoveList, position: *const Position, valid_destinations: SquareSet, comptime stm: Color) void {
    ...
    if (diag_promo != 0) {
        moves.pushPawnPromoCapture(promoable_base, diag_promo, diag_dir, .promo_q);
        moves.pushPawnPromoCapture(promoable_base, diag_promo, diag_dir, .promo_n);
        moves.pushPawnPromoCapture(promoable_base, diag_promo, diag_dir, .promo_r);
        moves.pushPawnPromoCapture(promoable_base, diag_promo, diag_dir, .promo_b);
    }
    if (anti_promo != 0) {
        moves.pushPawnPromoCapture(promoable_base, anti_promo, anti_dir, .promo_q);
        moves.pushPawnPromoCapture(promoable_base, anti_promo, anti_dir, .promo_n);
        moves.pushPawnPromoCapture(promoable_base, anti_promo, anti_dir, .promo_r);
        moves.pushPawnPromoCapture(promoable_base, anti_promo, anti_dir, .promo_b);
    }
}
